...
int main()
{
  ...
  看 b 的汇编，是先确定 e2 的大小。
  当 s2 是16 的倍数时， e2 最小为0，且n是奇数的时候， e1+e2=24，所以 e1 最大是 24。
  那么能不能 e2 为 16，e1 为 0 呢？ 不行，因为 s2 是 16 的倍数，根据 p 的计算方式，e2 肯定为0.

  当 s2 不是 16 的倍数时，e2 最大为15，且n是奇数的时候，e1 + e2 = 16，所以 e1 最小为1.
